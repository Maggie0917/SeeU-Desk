...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const article = await withDbRetry(() => prisma.article.findFirst({
    ...
    if (target === "summary") {
      ...
      const keywords = await generateKeywords(article.title, article.content);
      const updated = await withDbRetry(() => prisma.article.update({
        where: { id: article.id, userId: user.id },
        data: { summary: summary.text, keywords }
      }));
