...
FEISHU_REDIRECT_URI="https://你的-netlify-site.netlify.app/api/feishu/callback"

NEXT_PUBLIC_APP_URL="https://你的-netlify-site.netlify.app"
APP_URL="https://你的-netlify-site.netlify.app"
...
1. `DATABASE_URL` 使用生产 PostgreSQL 连接串，优先推荐 Netlify + Prisma Postgres 或 Neon PostgreSQL，也可使用 Supabase PostgreSQL、Railway PostgreSQL。
...
当前仓库已包含 PostgreSQL 初始 migration，可对空 PostgreSQL 数据库执行 `npx prisma migrate deploy`。不要在生产环境使用 `prisma db push` 作为长期迁移方案。
...
如果部署到 Netlify 等 serverless 平台，默认的 `macos_vision` OCR 不适合作为生产能力。请优先接入稳定的云 OCR 服务，并确保不在日志中打印用户图片内容或隐私信息。

如果生产环境未配置云 OCR，截图导入不应显示 mock 成功，应提示用户手动粘贴正文或联系管理员配置 OCR 服务。

## 8.1 Netlify 部署兼容性

当前项目可以作为 Next.js 应用部署到 Netlify，但上线前必须处理这些点：

1. 不要使用 `prisma/dev.db`，生产必须连接 PostgreSQL。
2. 不依赖 `.next`、`node_modules` 或本地数据库文件持久化。
3. 飞书 OAuth callback 是 Next.js Route Handler，可在 Netlify Next.js Runtime / Functions 环境运行。
4. Word / Markdown 导出当前通过响应内容生成，不依赖长期本地文件持久化。
5. PDF 如果走浏览器打印保存方案，需要在前端完成，不应依赖 serverless 本地写文件。
6. OCR 默认 `macos_vision` 不适合 Netlify，生产建议配置云 OCR Provider。
...
不在 `netlify.toml` 中写真实密钥。Netlify 环境变量应在 Netlify 站点后台配置，并确保需要参与构建的变量作用域包含 Builds。
...
6. 在 Netlify 项目后台配置生产环境变量。
7. 连接 PostgreSQL 数据库。优先推荐 Netlify + Prisma Postgres 或 Neon PostgreSQL。
8. 首次部署前对生产数据库执行 `npx prisma migrate deploy`。
9. 完成首次 Deploy。
10. 获取 Netlify 域名，例如 `https://seeu-desk.netlify.app`。
11. 将 `https://seeu-desk.netlify.app/api/feishu/callback` 添加到飞书开放平台 Redirect URL。
12. 回到线上产品设置页测试飞书授权。
13. 完成线上全流程验收。

如果后续绑定正式域名，例如 `https://seeudesk.com`，还需要：

1. 在 Netlify 中绑定正式域名。
2. 将 `FEISHU_REDIRECT_URI` 改为 `https://seeudesk.com/api/feishu/callback`。
3. 将同一个 URL 添加到飞书开放平台 Redirect URL。
4. 让用户重新连接飞书。
